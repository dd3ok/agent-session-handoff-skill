...
This failure is recorded, so `VALIDATION_RECORDED: yes` is appropriate. `SAFE_FOR_NEW_SESSION: yes` is still allowed because the next step is narrow, the failing command is known, and no command is still running.
